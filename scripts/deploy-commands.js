...
if (!clientId) {
  ...
  logError(
    'FATAL: DISCORD_BOT_USER_ID is missing in config. Cannot deploy commands.',
  )
  process.exit(1)
}
if (!guildId) {
  logError(
    'FATAL: GUILD_ID is missing in config. Cannot deploy guild commands.',
  )
  process.exit(1)
}
...
const commands = [...commandData, showCommand.toJSON()]
