...
class MockClient extends EventEmitter {
  constructor(options) {
    ...
    this.channels = {
      ...
      fetch: async (id) => {
        ...
        console.warn(
          `MockClient.channels.fetch: Channel ${id} not found in mock cache.`,
        )
    ...
    this.guilds = {
      ...
      fetch: async (id) => {
        ...
        console.warn(
          `MockClient.guilds.fetch: Guild ${id} not found in mock cache.`,
        )
        return null
      },
    }
  ...
  login(token) {
    console.log(
      `MockClient login called with token: ${token ? '***' : 'undefined'}`,
    )
...
function createMockGuildMember(override = {}) {
  const defaults = {
    ...
    user: {
      id: override.id || `mock-user-${Date.now()}`,
      tag: 'MockUser#0000',
      username: 'MockUser',
    },
    roles: {
