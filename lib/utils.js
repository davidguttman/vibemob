...
export function splitMessage(text, maxLength = DISCORD_MAX_MESSAGE_LENGTH) {
  ...
  log(`Splitting message of length ${text.length} into chunks of max ${maxLength}`);
  const chunks = [];
  let currentChunk = '';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentChunk.length + line.length + 1 > maxLength) {
      ...
      if (currentChunk.length > maxLength) {
         log(`Current chunk too long (${currentChunk.length}), performing hard split.`);
  ...
  if (currentChunk.length > 0) {
    ...
    if (currentChunk.length > maxLength) {
        log(`Final chunk too long (${currentChunk.length}), performing hard split.`);
        for (let j = 0; j < currentChunk.length; j += maxLength) {
            chunks.push(currentChunk.substring(j, j + maxLength));
        }
    } else {
