...
async function handleIncomingMessage({ message, userId }) {
    ...
    try {
        ...
        log('>>> DETAILED Aider Options for sendPromptToAider (Core):', JSON.stringify(aiderOptions, null, 2));
        
        const aiderResult = await aiderService.sendPromptToAider(aiderOptions);

        log(`Received full Aider result for ${userId}:`, JSON.stringify(aiderResult, null, 2));
        return aiderResult && aiderResult.stdout ? aiderResult.stdout : JSON.stringify(aiderResult);
    } catch (error) {
        logError(`Error calling Aider for user ${userId}:`, error);
        return `Error: Aider call failed. ${error.message}`;
    }
}
...
export const coreService = {
  ...
  // Expose state for testing if necessary (use cautiously)
  _getState: () => coreState, // Example: for test assertions
  _clearState: () => { // Example: for test cleanup
    coreState = {
      repoPath: null,
      isInitialized: false,
      currentModel: DEFAULT_MODEL,
    ...
    log('Core state manually cleared.');
  }
}; 