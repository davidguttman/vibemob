...
async function initializeCore({ repoPath }) {
  ...
  coreState = {
    ...
    currentModel: DEFAULT_MODEL,
    aiderInstance: null,
    contextFiles: [], // Reset state with new structure
  };
...
async function handleIncomingMessage({ message, userId }) {
    ...
    if (addMatch) {
        ...
        try {
            ...
            if (stats.isDirectory()) {
                ...
                filesToAdd.forEach(file => {
                    ...
                    if (!coreState.contextFiles.some(ctxFile => ctxFile.path === file)) {
                        // Add with readOnly: true by default for now
                        coreState.contextFiles.push({ path: file, readOnly: true });
                        addedCount++;
                    } else {
                        skippedCount++;
                    }
                ...
                if (coreState.contextFiles.some(ctxFile => ctxFile.path === relativePath)) {
                    ...
                    coreState.contextFiles.push({ path: relativePath, readOnly: true });
        ...
        const initialLength = coreState.contextFiles.length;

        // Filter out the file/path to remove
        coreState.contextFiles = coreState.contextFiles.filter(f => f.path !== relativePath);

        if (coreState.contextFiles.length < initialLength) {
            log(`Removed ${relativePath} from context. Current context:`, coreState.contextFiles);
            return `Removed ${relativePath} from the chat context`;
        } else {
            log(`Path ${relativePath} not found in context for removal.`);
            return `${relativePath} was not found in the chat context.`;
        }
        // ADD: Re-initialize aider service wrapper after removal to clear potential implicit context
        log('Re-initializing Aider service wrapper after context removal...');
        const aiderOptionsReinit = {
            repoPath: coreState.repoPath,
            modelName: coreState.currentModel,
        ...
        try {
            coreState.aiderInstance = await aiderService.initializeAider(aiderOptionsReinit);
            log('Aider service wrapper re-initialized.');
        } catch (reinitError) {
            logError('Failed to re-initialize Aider after removal:', reinitError);
            // Decide how to handle this - maybe return an error message?
            // For now, log the error and continue, the next call might fail.
        }
    } else if (clearMatch) {
        const initialLength = coreState.contextFiles.length;
        coreState.contextFiles = [];
        log(`Context cleared. Was ${initialLength} items, now ${coreState.contextFiles.length}.`);
    ...
    const editableFiles = coreState.contextFiles
        .filter(f => !f.readOnly)
        .map(f => f.path);
    const readOnlyFiles = coreState.contextFiles
        .filter(f => f.readOnly)
        .map(f => f.path);
    ...
    if (readOnlyFiles.length > 0) {
        ...
        for (const roFile of readOnlyFiles) {
            try {
                const filePath = path.join(coreState.repoPath, roFile);
    ...
    const aiderOptions = {
        ...
        files: editableFiles, // Only pass editable files to Aider's -f option
    };

    try {
        // ADD: Log the options being sent to Aider for verification
        log('Sending options to aiderService.sendPromptToAider:', JSON.stringify(aiderOptions, null, 2));
        ...
        log(`Received full Aider result for ${userId}:`, JSON.stringify(result, null, 2)); // Log the full structure
        // TODO: Implement read-only check here using the 'result' object structure
        return result.stdout || 'No output received'; // Return stdout for now
    } catch (error) {
