...
function _constructPromptWithChatHistory(userState, currentPrompt) {
  if (!userState.chatHistory || userState.chatHistory.length === 0) {
    return currentPrompt
  }
  
  let contextPrompt = "Here's our conversation so far:\n\n"
  
  userState.chatHistory.forEach(message => {
  ...
  log(`Constructed prompt with chat history context (${userState.chatHistory.length} messages)`)
  return contextPrompt
}

// Prepares options and runs the Aider interaction
async function _prepareAndRunAider(userId, prompt, useQAMode = false) {
  ...
  // Use chat history for context if in planning session
  const finalPrompt = userState.isPlanningSessionActive 
    ? _constructPromptWithChatHistory(userState, prompt)
    : prompt

  log(
    `Preparing to run Aider for user ${userId} with model ${userState.currentModel}${useQAMode ? ' in Q&A mode' : ''}`,
  )
  ...
  // Run Aider in Q&A mode or regular mode based on the flag
  try {
    let aiderResult
    
    if (useQAMode) {
      log(`Using Q&A mode for planning session`)
      const textResponse = await aiderService.sendQAPromptToAider({
        repoPath: globalRepoPath,
        prompt: finalPrompt,
        modelName: userState.currentModel,
        apiBase: userState.apiBase,
        apiKey: userState.apiKey,
        contextFiles: allContextFiles,
      })
      
      aiderResult = { content: textResponse }
    } else {
      aiderResult = await aiderService.sendPromptToAider({
        repoPath: globalRepoPath,
        prompt: finalPrompt,
        modelName: userState.currentModel,
        apiBase: userState.apiBase,
      ...
      aiderResult = { content: aiderResult.stdout }
    }
    
    // Log the result structure for debugging
    log(`Received Aider result for user ${userId}:`, aiderResult)
...
export async function handleIncomingMessage({ message, userId }) {
  ...
  if (userState.isPlanningSessionActive) {
    ...
    if (userState.currentPhase === 'planning-conversation') {
      try {
        const aiderResult = await _prepareAndRunAider(userId, message, true)
    ...
    try {
      const aiderResult = await _prepareAndRunAider(userId, message, false)
      return aiderResult // Return the structured response { content: ... }
    } catch (error) {
      // Return an error structure that can be relayed
      return { content: `Error processing your request: ${error.message}` }
