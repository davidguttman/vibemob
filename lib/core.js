...
async function _handleAddCommand(userId, relativePath, readOnly = true) { // Added readOnly param
  const userState = getUserState(userId)
  ...
  if (path.isAbsolute(relativePath) || relativePath.includes('..')) {
    ...
    return { message: `Error: Invalid file path '${relativePath}'. Please provide a relative path within the repository.` }
  }
  ...
  try {
    ...
    if (stats.isDirectory()) {
      ...
      if (filesToAdd.length === 0) {
        return { message: `Directory ${relativePath} is empty or contains no files.` }
      }
      ...
      return { message: `Added directory ${relativePath} to the chat context${readOnly ? ' (read-only)' : ''}.` }
      ...
      if (
        ...
        return { message: `Added ${relativePath} to the chat context${readOnly ? ' (read-only)' : ''}.` }
      }
    } else {
      logError(`Path is not a file or directory: ${relativePath}`)
      return { message: `Error: Path '${relativePath}' is not a valid file or directory.` }
    }
  } catch (error) {
    if (error.code === 'ENOENT') {
...
export async function handleIncomingMessage({ message, userId }) {
  ...
  // NOTE: Text-based commands like /add are deprecated in favor of slash commands
  // This function will now primarily handle prompts for Aider
  // const parsedCommand = _parseCommand(message)
  // if (parsedCommand) { ... }

  // Treat all incoming messages (not handled as slash commands) as prompts
  try {
...
export async function pushChanges({ userId /* Optional: for logging/context */ }) {
  if (!isCoreInitialized) {
  ...
  try {
    ...
    return { message: `Successfully pushed changes to branch ${config.workingBranch}.` }
  } catch (error) {
