...
async function pushChanges({ userId /* Optional: for logging/context */ }) {
  ...
  log(`Pushing changes for branch ${WORKING_BRANCH} in repo ${globalRepoPath} (Triggered by user: ${userId || 'Unknown'})`);
  ...
  try {
    ...
    return { pushed: true, branch: WORKING_BRANCH, details: pushResult };
  } catch (error) {
    logError(`Error pushing changes for user ${userId || 'Unknown'}:`, error);
    // Re-throw or return a structured error
    throw new Error(`Failed to push changes to ${WORKING_BRANCH}: ${error.message || error}`);
  }
}
