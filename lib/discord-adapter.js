...
async function _handleAutocomplete(interaction) {
  ...
  console.log(`Autocomplete triggered for command: ${commandName}, option: ${focusedOption.name}, value: "${focusedValue}", user: ${userId}`)
  try {
  ...
  try {
    ...
    console.log(`Successfully responded to autocomplete for ${commandName}:${focusedOption.name}`)
