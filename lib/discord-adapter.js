...
if (!BOT_TOKEN) {
  console.error(
    'FATAL: DISCORD_BOT_TOKEN is missing or empty in config. Cannot start.',
  )
  process.exit(1)
}
...
client.once(Events.ClientReady, async (c) => {
  ...
  if (loggingChannelId) {
    try {
      ...
      if (
        loggingChannel &&
        (loggingChannel.isTextBased() || loggingChannel.isThread())
      ) {
        log(
          `Logging channel fetched: #${loggingChannel.name} (${loggingChannel.id})`,
        )
        await sendLog('info', 'Bot restarted and logging initialized.')
      } else {
        logError(
          `Logging channel ID ${loggingChannelId} is not a text channel.`,
        )
        loggingChannel = null
      }
    } catch (err) {
...
async function _relayCoreResponse(channel, coreResponse, userId) {
  ...
  try {
    const diffRegex =
      /([\s\S]*?)^<<<<<<< SEARCH[\s\S]*?=======[\s\S]*?>>>>>>> REPLACE\s*$/gm
    let lastIndex = 0
    let match
    let normalContent = ''
    const filesToSend = []
    ...
    while ((match = diffRegex.exec(coreResponse)) !== null) {
      ...
      const linesBeforeDiff = precedingText
        .split('\n')
        .filter((l) => l.trim() !== '')
      const fileNameGuess =
        linesBeforeDiff.length > 0
          ? linesBeforeDiff[linesBeforeDiff.length - 1].trim()
          : 'aider_change.diff'
      const safeFileName =
        fileNameGuess.replace(/[^a-zA-Z0-9_.-]/g, '_') || 'aider_change.diff'
      const diffFileName = safeFileName.endsWith('.diff')
        ? safeFileName
        : `${safeFileName}.diff`
      ...
      filesToSend.push(
        new AttachmentBuilder(diffBuffer, { name: diffFileName }),
      )
...
function buildEmbedsFromTokens(tokens) {
  ...
  const finalize = (nextLen = 0) => {
    if (
      ...
      (currentDescription.length + nextLen > MAX_EMBED_DESCRIPTION_LENGTH ||
        totalLength + nextLen > MAX_TOTAL_EMBED_LENGTH)
    ) {
      ...
      switch (t.type) {
        ...
        case 'paragraph_open': {
          ...
          while (j < tokens.length && tokens[j].type !== 'paragraph_close') {
            ...
            else if (inTok.type === 'softbreak' || inTok.type === 'hardbreak')
              paragraph += '\n'
            j++
          }
...
async function _handleInitialMention(message) {
  ...
  try {
    thread = await message.startThread({
      name: `${THREAD_NAME_PREFIX} ${message.author.username}`.substring(
        0,
        100,
      ),
  ...
  try {
    ...
    const initialPrompt = botMention
      ? message.content
          .replace(`<@${botMention.id}>`, '')
          .replace(`<@!${botMention.id}>`, '')
          .trim()
      : message.content.trim()
    await thread.send(
      `Okay, ${message.author.username}. Working on: "${initialPrompt}"`,
    )
    await thread.sendTyping()
    const coreResponse = await coreService.handleIncomingMessage({
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (
    ...
    message.channel.name.startsWith(THREAD_NAME_PREFIX)
  ) {
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    if (commandName === 'model') {
      ...
      await interaction.deferReply({ ephemeral: true })
      const res = await coreService.handleIncomingMessage({
        message: `/add ${pathToAdd}`,
        userId,
      })
  ...
  if (loggingChannel?.isTextBased()) {
    for (const chunk of splitMessage(discordMessage, 2000)) {
      await loggingChannel
        .send(chunk)
        .catch((e) => logError('Failed to send log chunk:', e))
    }
  }
}
