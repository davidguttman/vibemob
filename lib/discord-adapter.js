...
const BOT_USER_ID = config.discordBotUserId; // Make sure this exists in config.js/env
...
client.on(Events.MessageCreate, async message => {
  logVerbose(`Received message event: ID=${message.id}, Author=${message.author.tag}, Bot=${message.author.bot}, Guild=${message.guildId}, Channel=${message.channelId}, Content (start): "${message.content?.substring(0, 50)}..."`); // Added verbose log

  // Ignore bots
  if (message.author.bot) {
    logVerbose(`Ignoring message ID ${message.id}: from bot ${message.author.tag}.`); // Added verbose log
    return;
  }
  // Ignore messages not in guilds (e.g., DMs) for now
  if (!message.guild) { 
    logVerbose(`Ignoring message ID ${message.id}: not in a guild (DM?).`); // Added verbose log
    return; 
  }
  ...
  if (message.channel.isThread() && message.channel.name.startsWith(THREAD_NAME_PREFIX)) {
    logVerbose(`Message ID ${message.id} is in a managed thread ${message.channel.id}. Handling as thread message.`); // Added verbose log
    // Potentially add check: && await message.channel.members.fetch(client.user.id) to ensure bot is member?
    await _handleThreadMessage(message);
    return; // Message handled
  }

  // Check if the bot was mentioned (using configured BOT_USER_ID)
  // Matches <@BOT_ID> or <@!BOT_ID> at the start of the message
  const mentionRegex = new RegExp(`^<@!?${BOT_USER_ID}>\s*`); 
  const mentionMatch = message.content.match(mentionRegex);

  if (mentionMatch) {
    logVerbose(`Message ID ${message.id} contains bot mention. Handling as initial mention.`); // Added verbose log
    await _handleInitialMention(message, mentionMatch);
  } else {
    logVerbose(`Ignoring message ID ${message.id}: does not start with bot mention.`); // Added verbose log
