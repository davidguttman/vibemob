  ...
  if (config.sshPrivateKeyB64 && process.env.NODE_ENV !== 'test') {
    if (!sshCommand) { 
      try {
        ...
        logSsh(`Creating temporary SSH key directory: ${tempSshDir}`);

        // Decode and write the key
        const keyContent = Buffer.from(config.sshPrivateKeyB64, 'base64').toString('utf8');
        await fs.writeFile(tempSshKeyPath, keyContent, { mode: 0o600 }); // IMPORTANT: Set mode 600
        logSsh(`Temporary SSH key written to: ${tempSshKeyPath}`);

        // Generate the SSH command using the temp key path
        sshCommand = `ssh -i ${tempSshKeyPath} -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null`;
        logSsh(`Generated GIT_SSH_COMMAND using temporary key.`);
        
      } catch (err) {
        logError('Failed to create or write temporary SSH key:', err);
        ...
        tempSshKeyPath = null;
        sshCommand = null;
        throw new Error(`Failed to configure SSH key: ${err.message}`); // Re-throw
      }
    }
    // Set the environment variable for the simple-git process
    options.env = { ...process.env, GIT_SSH_COMMAND: sshCommand };
