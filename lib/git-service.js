...
`
        ...
        await fs.writeFile(tempSshConfigPath, sshConfigContent.trim(), {
          mode: 0o600,
        })
        ...
        if (tempSshDir) {
          await fs
            .rm(tempSshDir, { recursive: true, force: true })
            .catch((e) =>
              logError(
                'Error cleaning up temp SSH dir after write failure:',
                e,
              ),
            )
        }
    ...
    logSsh(
      'Running in test environment, assuming SSH is handled by Docker env/config.',
    )
  } else {
...
async function checkoutBranch({ localPath, branchName }) {
  ...
  try {
    ...
    logError(
      `Error checking out branch '${branchName}' in ${localPath}:`,
      error,
    )
    throw error
  }
...
async function pushBranch({ localPath, branchName }) {
  ...
  try {
    ...
    log(
      `Successfully pushed branch '${branchName}' to origin from ${localPath}`,
    )
  } catch (error) {
...
async function checkoutOrCreateBranch({ localPath, branchName }) {
  ...
  try {
    ...
    if (localExists) {
      ...
      if (remoteExists) {
        log(
          `Remote branch 'origin/${branchName}' also exists. Fetching and resetting.`,
        )
        await git.fetch('origin')
        ...
        log(
          `Branch '${branchName}' exists locally but not remotely. Keeping local state.`,
        )
      }
      ...
      log(
        `Branch '${branchName}' does not exist locally or remotely, creating from ${mainBranchRef}.`,
      )
    ...
    logError(
      `Error during checkout/create branch '${branchName}' in ${localPath}:`,
      error,
    )
    throw error
  }
...
async function deleteRemoteBranch({ localPath, branchName }) {
  const git = await _getGitInstance(localPath)
  log(
    `Attempting to delete remote branch 'origin/${branchName}' from ${localPath}`,
  )
  try {
    await git.push('origin', ['--delete', branchName])
    log(
      `Successfully requested deletion of remote branch 'origin/${branchName}'`,
    )
  } catch (error) {
    if (error.message && error.message.includes('remote ref does not exist')) {
      log(
        `Remote branch '${branchName}' likely did not exist or was already deleted.`,
      )
    } else {
      logError(
        `Error deleting remote branch '${branchName}' from ${localPath}: ${error.message}`,
      )
    }
  }
}
